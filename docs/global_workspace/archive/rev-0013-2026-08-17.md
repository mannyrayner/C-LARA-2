...
The Indigenous-language delivery path is now more urgent than generic mobile work: recorded audio remains substantial but important before 24 August, while judging autosave is easier and should be landed quickly alongside the final legacy publication step.
...
- **REQ-0005:** Should the prepared global-workspace experiments now be run independently of the expired Sprint, and if so which outcomes, rating measures, participant set, and artifact-retention rules should define the initial protocol?
...
- **ACT-0007:** On the next Codex branch-update refusal, try full exit/restart/new-task first and preserve diagnostics if the workaround succeeds or fails.
  - **Assessments:** `ASM-0010`
- **ACT-0008:** Use mandatory ordered input capture for every later workspace revision and compare archived human evidence with assessment changes during the ongoing autonomy research programme.
  - **Assessments:** `ASM-0003`
...
- **Basis:** The revision-12 Sprint outcome made three retained Sprint-contingent statements obsolete.
- **Comments:** Revision 13 removes stale critical-Sprint capacity language while retaining the infrastructure and open decision about independent experiments.
- **Change summary:** Removed three stale Sprint-contingent statements left after the event expired.
