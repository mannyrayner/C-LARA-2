...
Pipeline stages run in a DjangoQ-style background task and write both to `progress.jsonl` and to the user's session so new messages surface while the job is in flight. If a progress entry cannot be converted into a UI message, the compile task logs a detailed exception via the `projects.views` logger. With the default settings in this repository, those log lines appear in the server console where you run `python manage.py runserver`. In deployed environments, check your configured Django logging handlers (e.g., Gunicorn or systemd service logs) for entries from `projects.views`.
