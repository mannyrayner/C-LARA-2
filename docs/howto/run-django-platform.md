...
Codex repository inspection also needs the CLI sandbox helper to work in the same environment. On Linux/WSL2, install `bubblewrap` so `bwrap` is on `PATH` (for example, `sudo apt-get install bubblewrap` on Debian/Ubuntu/WSL2). Native Windows/Cygwin shells may report `bwrap=(not found)` and then fail Assistant requests with the same "Linux sandbox/command execution layer failed" message, even for simple queries such as "Summarise the project briefly." If setting up WSL2/Linux locally is too much work for a one-off investigation, skip laptop reproduction and run the server diagnostics in `docs/howto/server-admin-tasks.md` instead; the production server already has the dedicated worker and Codex sandbox mostly working, so it is the better place to compare normal repository questions with Assistant self-queries.
