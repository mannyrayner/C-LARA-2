...
- Updated local real-queue startup guidance so `make run-platform-with-real-q` also starts the dedicated project-understanding worker, and documented that Windows/Cygwin laptop testing still needs a working Codex/bubblewrap sandbox, preferably through WSL2/Linux.
...
- **ISSUE-0034** now needs focused production diagnostics: the Assistant worker can answer many repository questions, but queries about the Assistant tab/self-understanding path consistently fail with a `failed rtm_newaddr` sandbox-layer error; first compare with a laptop run under `make run-platform-with-real-q`, but only from an environment where Codex can find/use `bwrap` for its read-only sandbox.
