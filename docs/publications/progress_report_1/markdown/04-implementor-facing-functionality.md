...
- A new English MWE pilot extends the same autonomy pattern to linguistic annotation beyond segmentation: after a human manually corrected seven development projects, the AI diagnosed stale `mwe_count` metadata, added a fixed-split refresh target, and proposed a development-only prompt-improvement loop over 336 segments, 5,104 tokens, and 140 manually corrected MWEs. This is a useful process vignette because the human supplies expert gold judgements while the AI maintains tooling, leakage controls, and report-facing documentation.
