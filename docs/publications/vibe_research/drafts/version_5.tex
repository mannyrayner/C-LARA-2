...
We then consider why comparable practices have been absorbed much more readily into software engineering than into academic publishing. One important difference is institutional. Software-development methodology can change directly through practice: a developer who finds a new agentic workflow useful can begin using it immediately. Academic publishing is governed by formal authorship and ethics policies which change through slower institutional processes. We examine the ACL Policy on Publication Ethics as a particularly relevant example. Its framework for generative assistance, introduced in 2024 and still substantially retained in the current policy, conceptualises AI primarily in terms of discrete generation of text and ideas rather than sustained project-scale agentic collaboration.
...
Finally, if the technical analogy is as close as we suggest, a further question arises. Why have the two communities developed such different norms? We will argue that part of the explanation may lie not in intrinsic differences between software and research, but in the mechanisms through which their working practices change. Software methodology can evolve directly through individual and organisational practice. Academic research is additionally constrained by formal systems of publication governance whose categories may change on a substantially slower timescale.
...
This provides a concrete reason for expecting high-end software agents to be useful research agents as well. No entirely new architecture is required. The same system which can inspect a software repository can inspect research materials stored beside it.
...
The experimental code, prompts, and artefacts are public \citep{raynerChatGPT2025grok}. The project context can also be placed in the same repository structure used for the preceding examples and exposed through the existing project interface.

The three cases consequently provide examples at markedly different scales: a large long-term software and research project, an exploratory longitudinal language-learning study, and a rapid empirical investigation prompted by a topical question.
...
Software development is governed by standards, organisational rules, security requirements, and legal constraints, but individual developers and teams generally have considerable freedom in choosing how they produce code. If a new coding agent proves useful, a developer can begin using it immediately. If the resulting workflow is effective, colleagues can imitate it. Working practice can therefore change on approximately the same timescale as the underlying technology.

The adoption of agentic coding assistants provides a clear example. As model capabilities improve, developers can alter their division of labour with the AI without first requiring a professional body to redefine what counts as programming.
...
The ACL Policy on Publication Ethics provides a particularly instructive example because ACL is one of the world's leading professional organisations in computational linguistics and natural-language processing. It would be difficult to select a research community more directly concerned with the technologies under discussion.

The policy was first approved in June 2024 \citep{acl2024policy}. Its section on generative assistance divides acceptable AI use into a small taxonomy: assistance with language, short-form input assistance, literature search, generation of low-novelty text, generation of new ideas, and ``new ideas + new text''. In the final category, the policy states that ACL does not consider a generative model capable of fulfilling the requirements of co-authorship \citep{acl2024policy}.

The current policy retains essentially the same conceptual framework \citep{acl2026policy}.

This is notable because the policy itself has not been static. The ACL Executive Committee records repeated approvals of the document in 2025, and publicly records amendments and additions, including substantial changes in areas such as scientific integrity and coordinated disclosure \citep{acl2026policy,aclresolutions2025}.
...
What it does establish is narrower and more interesting: a policy document which has continued to receive institutional attention retains a model of generative AI originally formulated in 2024.

That model is recognisably generation-centric.

It asks whether the system assists with wording, produces low-novelty text, suggests new ideas, or produces new ideas together with new text. These are natural categories for a chatbot-style interaction in which a user supplies a prompt and receives content.

They do not map naturally onto the project-scale agentic workflow described earlier in this paper.

Consider, for example, an AI agent which over several weeks or months can inspect a research repository, modify code, run an experiment, examine its output, notice a failure, revise the analysis, update documentation, discuss the interpretation with a human collaborator, remember the resulting project state, and later return to the artefacts to explain why a particular decision was made.

Calling this ``new ideas + new text'' is not exactly false. It is simply too coarse a description to capture the relevant division of labour.

The difference is important because the technological progression from conversational LLMs to tool-using assistants and then to persistent project agents is not merely quantitative. Each stage changes what it is practical to delegate and what forms of collaboration are possible.

A policy formulated primarily in terms of generated content can therefore remain internally coherent while becoming progressively less descriptive of actual working practice.
...
Individual software developers can adapt their practice whenever a new tool becomes useful. A publication policy cannot normally change in the same informal way. It must be proposed, discussed, approved, documented, implemented in submission systems, communicated to venues, and applied consistently.
...
But unusually rapid technological change creates a difficult tradeoff. If the capabilities being governed change from text generation to tool use to persistent autonomous project work over a period of only a few years, a deliberately slow governance process may end up regulating a model of interaction which no longer corresponds well to the frontier practice it is intended to cover.
...
Evidence from professional writing suggests another complication: actual use and public disclosure may diverge when social norms remain unsettled.

In a 2026 interview with Casey Newton, Dan Shipper argued that AI use among professional writers was much more widespread than writers publicly admitted. In the same interview he distinguished between communities already accustomed to AI as an everyday working technology and mainstream audiences more sensitive to its use \citep{newton2026editorclone}. This is anecdotal evidence about professional writing rather than academic research, and should be treated as such. It nevertheless illustrates a plausible mechanism by which observable disclosure can lag behind actual practice.
...
The ACL taxonomy discussed above illustrates the problem. A researcher using a persistent agentic workflow may be required to describe the collaboration using categories designed around generated content, even when no category accurately represents the process.
...
Software developers can often alter their working methodology immediately when a new tool becomes useful. Academic researchers who wish to publish must operate within governance frameworks which define authorship and acceptable AI assistance.

The ACL publication-ethics policy provides a concrete example. Its current treatment of generative assistance retains the basic taxonomy already present in the original 2024 policy, even though the policy as a whole has subsequently undergone repeated review and revision \citep{acl2024policy,acl2026policy,aclresolutions2025}. The taxonomy is well suited to a generation-centric model of AI assistance but maps less naturally onto persistent project-scale agentic collaboration.
