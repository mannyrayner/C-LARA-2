...
10. **Then: evaluate and compare.** Implement/fill `evaluate`, `compare`, and `report` so the experiment produces paired default-vs-candidate judgements, flagged examples, corpus-split-aware summaries, and a concise Markdown result suitable for maintainer review and possible progress-report evidence.
