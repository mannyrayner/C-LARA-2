...
7. **Implemented human spot-check target: judge segmentation outputs.** `judge-default` and `judge-candidate` call `judge_segmentation_outputs`, which displays each output as a compact `Input surface` / `Segments` prompt, appends judgements continuously for interruption-safe review, and shares a cache keyed by input surface plus token sequence so repeated identical segmentations are not judged twice while still being copied into the current run's judgement file for later comparison. This gives the human supervisor an efficient development-audit loop before the automated evaluator/comparison targets are finalized.
8. **Then: evaluate and compare.** Implement/fill `evaluate`, `compare`, and `report` so the experiment produces paired default-vs-candidate judgements, flagged examples, corpus-split-aware summaries, and a concise Markdown result suitable for maintainer review and possible progress-report evidence.
