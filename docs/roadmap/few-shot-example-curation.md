...
10. **Implemented first comparison layer.** `compare_segmentation_judgements` powers the Makefile's `evaluate` and `compare` targets over human judgement JSONL files. It takes the latest judgement per record, compares default against one or more candidate tranches, emits JSON/Markdown summaries, and writes flagged candidate wins/losses/disagreements for review.
11. **Then: report and test freeze.** Use the development comparison summaries to choose one candidate setting and comparison rule, then run the held-out test default/candidate pair once and build `report` from the frozen comparison outputs.
