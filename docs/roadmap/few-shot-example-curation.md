...
4. **Next: create deterministic corpus manifests.** Add Make targets and a management command/helper that use `corpus_summary.json` to create `generated/corpus_splits/development.jsonl` and `generated/corpus_splits/test.jsonl`, preferably with project-level separation and a capped/stratified segment sample. The development split should be used for prompt/evaluator adjustment; the test split should remain untouched until the comparison procedure is fixed.
