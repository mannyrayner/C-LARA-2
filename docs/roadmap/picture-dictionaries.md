...
- UX cleanup implemented (2026-06-17): the unified entry rows are grouped into a narrower two-tier layout, with Select all, Select incomplete, Select none, Display all, and Display incomplete only controls at the top of the table. Incomplete rows now include entries missing lemma, POS, gloss/translation, prompt, or image. The older placeholder-sync/compile controls have been removed from the organiser page in favour of direct dictionary operations, and selected rows can now ask AI to create missing lemma/POS/translation information, missing/default prompts, and missing images as needed; surface-only prompts are only considered missing when the row has no image, to avoid separating a liked image from the prompt that produced it. The workspace now also exposes a dictionary-level translation language, syncs it to the associated project/subset projects, supports Create translations for selected rows, uses fan-out/fan-in for selected-row AI text operations, exposes a standard-menu image prompt language control (defaulting to source language when AI-supported and otherwise translation language), tells prompt construction to use POS plus translation/gloss for disambiguation, and now fans out page-image prompt construction as well as image rendering. The organiser page has also been simplified around the source-of-truth model: the dictionary is ensured automatically, the old page-oriented review/compile controls and image review dashboard are hidden, exercise creation/testing links are available from the unified dictionary and subset sections, created exercise play links are also exposed on the community member page, exercise backlinks return to the community page that launched them, and saved subsets show read-only word/gloss/image previews. Next, make this workspace more useful for Sophie testing by adding richer prompt-variant management, clearer image-generation progress feedback, and tighter exercise/subdictionary integration from the same selected-row workspace.
