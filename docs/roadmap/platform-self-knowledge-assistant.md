...
### Phase D: dedicated worker service — first cut implemented and verified on AWS
...
- AWS deployment now uses a dedicated systemd worker unit running as `ubuntu:www-data`, with explicit `HOME=/home/ubuntu`, the proven Codex/AppArmor/CODEX_HOME environment, and the queue worker command. The current unit shape is:
...
For one queued request on a laptop or during debugging, use `python manage.py process_project_understanding_queue --once`; if it fails with `failed rtm_newaddr`, first confirm `python manage.py check_project_understanding_codex --smoke` works in that same shell. A `failed rtm_newaddr` result means the local Codex/bubblewrap sandbox still cannot inspect the repository from that process context; it is not a queue bug. If the server can answer normal repository questions but fails only when the answer mentions the Assistant/issue-tracking text, suspect a detector false positive before assuming bubblewrap failed: the sandbox detector is intended to flag direct live-access failures such as `bwrap: loopback: Failed RTM_NEWADDR`, not successful answers that quote `docs/issues` prose about past sandbox errors.

- Server testing on 2026-06-14 confirmed that the dedicated worker can process live Assistant requests end to end. Still needed after server testing: add stale-lock/stale-running recovery if a worker is killed mid-run, and decide whether the file-backed queue should later be replaced with a database-backed queue model.
