...
- Add concrete examples for picture dictionaries, picture-clue word scrambles as rapid user-requested functionality, low-resource-language support, AWS migration, the restricted project-understanding assistant, and first-version AI evaluation of default versus candidate linguistic phase outputs. For the word-scramble example, explicitly separate the rapid AI/human-expert platform implementation estimate from Sophie’s end-user/community consultation work, and ask Sophie what, if anything, can be said publicly given the sensitivity of Australian Aboriginal language work.
