...
- `docs/publications/eurocall_2026_voice_mode/` — repository source and approved session-summary evidence for the ChatGPT Voice Mode German-practice case study.

## Current status snapshot

- **First progress report:** posted on ResearchGate on **2026-07-23**; completed under
...
- Recent picture-dictionary work provides an additional strong report example: in a short AI/human steering loop, the platform gained (i) correct propagation of “disallow visible text in images” into organiser-requested image regeneration, which is essential for image-based flashcards, and (ii) AI-based low-resource dictionary consistency diagnostics that catch likely word/gloss language-confusion errors and surface a review trace. This is a useful case study for the claim that substantial user-facing functionality can be added quickly in an AI-centered platform while remaining under human domain-expert review.
...
- **Companion Voice Mode paper:** `docs/publications/eurocall_2026_voice_mode/` preserves the case study of ChatGPT Voice Mode for spoken German practice. Sarah Wright has approved adding the session summaries to the public repository; Markdown summaries are being added with source, conversion, permission, and evidentiary-status metadata so that the C-LARA-2 Assistant can relate them to the paper without treating them as transcripts. The final proceedings editors and DOI remain to be supplied.
