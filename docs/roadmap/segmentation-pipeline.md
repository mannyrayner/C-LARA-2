...
Contract: Every in-flight API op emits heartbeat events at ~5s cadence. The core pieces live under `core/` (sibling to `docs/`) so they are easy to import from the CLI and future Django app.
