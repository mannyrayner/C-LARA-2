...
class DeriveFewshotAssetsTests(SimpleTestCase):
    ...
    def test_command_derives_evaluator_only_assets(self):
        with tempfile.TemporaryDirectory() as tmp:
            repo_root = Path(tmp)
            curation_root = repo_root / "generated" / "few_shot_curation"
            root = curation_root / "segmentation_phase_2" / "fr" / "boundary_first" / "clitic_compound_v2_evaluator"
            candidates_dir = root / "candidates"
            reviews_dir = root / "reviews"
            candidates_dir.mkdir(parents=True)
            reviews_dir.mkdir(parents=True)
            candidate_path = candidates_dir / "20260622-000000Z-EXAMPLE-0001.json"
            candidate_path.write_text(
                json.dumps(
                    {
                        "example_id": "EXAMPLE-0001",
                        "candidate": {
                            "input": "C'est juste.",
                            "phenomenon": "French clitic",
                            "rationale": "Evaluator calibration example.",
                            "output": {
                                "surface": "C'est juste.",
                                "tokens": [
                                    {"surface": "C'"},
            ...
            (reviews_dir / "20260622-000000Z.items.json").write_text(json.dumps(items), encoding="utf-8")
            (reviews_dir / "20260622-000000Z.human_audit.jsonl").write_text(
                json.dumps({"example_id": "EXAMPLE-0001", "human_judgement": "correct"}) + "\n",
                encoding="utf-8",
            )
            evaluator_jsonl = repo_root / "generated" / "derived_assets" / "evaluator" / "evaluator_examples.jsonl"
            manifest_json = repo_root / "generated" / "derived_assets" / "evaluator" / "manifest.json"

            call_command(
                "derive_fewshot_assets",
                asset_kind="evaluator",
                operation="segmentation_phase_2",
                language="fr",
                mechanism="boundary_first",
                target_set="clitic_compound_v2_evaluator",
                request_id="20260622-000000Z",
                repo_root=str(repo_root),
                curation_root=str(curation_root),
                evaluator_output_jsonl=str(evaluator_jsonl),
                manifest_json=str(manifest_json),
            ...
            evaluator_payload = json.loads(evaluator_jsonl.read_text(encoding="utf-8").splitlines()[0])
            manifest = json.loads(manifest_json.read_text(encoding="utf-8"))
            self.assertEqual(evaluator_payload["boundary_marked"], "C'¦est¦ ¦juste¦.")
            self.assertEqual(manifest["processing_output_dir"], "")
            self.assertEqual(manifest["evaluator_output_jsonl"], str(evaluator_jsonl))
