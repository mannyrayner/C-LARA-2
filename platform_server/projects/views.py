...
def reports_home(request: HttpRequest) -> HttpResponse:
    updates_dir = _reports_root() / "updates"
    updates_dir.mkdir(parents=True, exist_ok=True)
    report_files = sorted(updates_dir.glob("*.html"), key=lambda p: p.name, reverse=True)
    reports = [
        {
            "name": path.name,
    ...
    return render(request, "projects/reports_home.html", {"reports": reports})


@login_required
def report_detail(request: HttpRequest, filename: str) -> HttpResponse:
    safe_name = os.path.basename(filename)
    if safe_name != filename or not safe_name.endswith(".html"):
        raise Http404()
    path = _reports_root() / "updates" / safe_name
    if not path.exists():
        raise Http404()
